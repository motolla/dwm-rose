...
drawbar(Monitor *m)
{
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    w = TEXTW(tags[i]);
    drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i || occ & 1 << i ? tagschemes[i] : SchemeNorm]);
...
void
runAutostart(void) {
  system("cd ~/dot/dwm-6.4/autostart; ./autostart_blocking");
  system("cd ~/dot/dwm-6.4/autostart; ./autostart &");
}
